...
/**
 * Servlet implementation class updateDeliverySchedule
 */
@WebServlet("/updateDeliveryScheduleServlet")
public class updateDeliveryScheduleServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public updateDeliveryScheduleServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int scheduleID = Integer.parseInt(request.getParameter("scheduleID").substring(0,request.getParameter("scheduleID").length()-1));
		int beneficiaryID = 0;
		...
		deliverySchedule.setScheduleID(scheduleID);
		...
		try{
			boolean success = deliveryScheduleManager.updateDeliverySchedule(deliverySchedule);
			if(success){
				rd = request.getRequestDispatcher("updateDeliveryScheduleDone.jsp");
			}
		}
		catch (Exception ex){
