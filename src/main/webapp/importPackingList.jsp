	...
	$(document).ready(function() {
		...
		$(".importBtn").click(function() {
			...
			if(rowIndex >= 0){
			 ...
			 $(window.opener.document).find("#eventLocation").val(eventLocation);
			 $(window.opener.document).find("#"+eventType).prop("checked",true);
			 $(window.opener.document).find("#amountOfBags").val(amountOfBags);
			 
			 var beverageCount=0;
			 var cannedFoodCount=0;
			 var generalFoodCount=0;
			 
			 $('tr.list'+correctRowIndex).each(function() {
				  $this = $(this);
				  ...
				  $("<tr id='"+insertId+count+"' class='"+insertId+"'><td>"+name+"<input type='hidden' name='"+insertId+"Name' value='"+name+"'></td><td>"+weight+"<input type='hidden' name='"+insertId+"Weight' value='"+weight+"'></td><td><input type='number' style='width:55%;' value='"+quantity+"' class='quantity'  min=0 max=99 step=1 name='"+insertId+"Quantity'></td><td><button class='deleteRow' type=button>Delete</button></td></tr>").insertBefore($(window.opener.document).find(insertBeforeId));
				});		
